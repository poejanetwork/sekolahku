    ...
    function init_template() {
        ...
        document.querySelectorAll('a').forEach((_link) => {
            _link.addEventListener('click', (e) => {
                ...
                const target = _link.getAttribute('target');
                if (
                    href === '#' ||
                    href === '' ||
                ...
                if (preloader) {
                    ...
                    if (menuMain["classList"]["contains"]('menu-active')) {
                        menuMain["classList"]["remove"]('menu-active')
                    };
        ...
        function _0x7231x6a() {
            ...
            _0x7231x6b["forEach"]((_0x7231xb) => {
                return _0x7231xb["addEventListener"]('click', (_0x7231xa) => {
                    ...
                    _0x7231x6f["href"] = base_url+'/theme/default/assets/styles/highlights/highlight_' + _0x7231x6d + '.css';
            ...
            if (_0x7231x69) {
                ...
                _0x7231x6f["className"] = "page-highlight";
                _0x7231x6f["type"] = "text/css";
                _0x7231x6f["href"] = base_url+'/theme/default/assets/styles/highlights/highlight_' + _0x7231x69 + '.css';
        ...
        var _0x7231xbbbb = document["querySelectorAll"]('[data-dismiss]');
        if (_0x7231xbbbb["length"]) {
            _0x7231xbbbb["forEach"]((_0x7231xbbb) => {
                return _0x7231xbbb["addEventListener"]('click', (_0x7231x82) => {
                    var _0x7231xbc = _0x7231xbbb["getAttribute"]('data-dismiss');
                    var _0x7231xbd = document["getElementById"](_0x7231xbc);
                    var _0x7231xbd = new bootstrap.Toast(_0x7231xbd);
                    _0x7231xbd["hide"]()
                })
        ...
        //
        var _0x7231xf31 = document["querySelectorAll"]('[data-search-users]');
        if (_0x7231xf31["length"]) {
            const input = document.querySelector('[data-search-users]');
            const resultBox = document.querySelector('#fullname-results');
            document.querySelector('[data-search-users]').addEventListener('input', function () {
                const query = this.value.trim();
                const url = this["getAttribute"]('data-geturl');
                ...
                .then(text => {
                    try {
                        ...
                        if (data.length === 0) {
                            ...
                            const html = data.map(siswa => `<a href="javascript:void(0)" class="bg-highlight list-group-item list-group-item-action mb-1 px-1 py-0" data-uid="${siswa.id}" data-nama="${siswa.fullname}">
                        ${siswa.fullname} <small class="text-muted">(${siswa.nisn})</small>
                    </a>`).join('');
        ...
        document.addEventListener('click', function(e) {
            if (e.target.matches('#fullname-results a')) {
                const resultBox = document.querySelector('#fullname-results');
                const nama = e.target.getAttribute('data-nama');
                const uid = e.target.getAttribute('data-uid');
                console.log(nama);
                console.log(uid);
                document.querySelector('#fullname').value = nama;
                document.querySelector('#uid').value = uid;
                resultBox.classList.add('d-none');
                resultBox.innerHTML = '';
            }

            if (!e.target.closest('#fullname') && !e.target.closest('#fullname-results')) {
            }
        });
        ...
        if (_0x7231xf3["length"]) {
            document.querySelector('[data-search]').addEventListener('input', function () {
                ...
                if (!resultBox || !noResults) return;

                if (query === '') {
                    resultBox.classList.add('disabled-search-list');
                ...
                fetch(url + '/' + encodeURIComponent(query))
                    ...
                    .then(data => {
                        ...
                        if (data.length === 0) {
                            ...
                            noResults.classList.add('disabled');
                            resultBox.classList.remove('disabled-search-list');

                            const html = data.map(siswa => `
                                <a href="${siswa.user_url}" data-card-height="80" class="card round-m m-0" style="height: 60px;">
                                <div class="card-center">
                                    <h5 class="">${siswa.fullname}</h5>
                                    <p class="mt-n2 font-12 color-highlight mb-0">NISN: ${siswa.nisn}</p>
                                </div>
                                <div class="card-center opacity-30">
                                    <i class="fa fa-arrow-right opacity-50 float-end color-theme pe-3"></i>
                                </div>
                                </a>`).join('');
                            resultBox.innerHTML = `<div class="card card-style mx-1 px-3 mt-0 mb-0 shadow-0">${html}</div>`;
                        }
                    });
                }
            });
        ...
        function _0x7231x42(_0x7231x117, _0x7231x3b, _0x7231x4a) {
            setTimeout(function() {
                if (_0x7231x3b === "show") {
                    return document["getElementById"](_0x7231x117)["classList"]["add"]('menu-active'), document["querySelectorAll"]('.menu-hider')[0]["classList"]["add"]('menu-active')
                } else {
                    return document["getElementById"](_0x7231x117)["classList"]["remove"]('menu-active'), document["querySelectorAll"]('.menu-hider')[0]["classList"]["remove"]('menu-active')
                }
            }, _0x7231x4a)
        }
